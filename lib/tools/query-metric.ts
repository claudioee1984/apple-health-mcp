...
export async function queryMetric(db: any, args: QueryMetricArgs) {
  ...
  if (aggregation === "raw") {
    const points = await db
      .select({ date: metricSamples.date, source: metricSamples.source, units: metricSamples.units, extra: metricSamples.extra, qty: metricSamples.qty, min: metricSamples.min, avg: metricSamples.avg, max: metricSamples.max, systolic: metricSamples.systolic, diastolic: metricSamples.diastolic })
